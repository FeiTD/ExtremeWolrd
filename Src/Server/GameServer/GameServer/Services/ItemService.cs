...
namespace GameServer.Services
{
    public class ItemService:Singleton<ItemService>
    {
        ...
        {
            Log.InfoFormat("OnItemBuy");
            Character character = sender.Session.Character;
            Log.InfoFormat("OnItemBuy::character:{0}: Shop:{1} ShopItem:{2}", character.Id, request.shopId, request.shopItemId);
            var reslut = ShopManager.Instance.BuyItem(sender,request.shopId, request.shopItemId);
            sender.Session.Response.itemBuy = new ItemBuyResponse();
            sender.Session.Response.itemBuy.Result = reslut;
            sender.Session.Response.itemBuy.Errormsg = "Error";
            sender.SendResponse();
        }
    }
}
